...
export default function PublicProfile() {
  ...
  useEffect(() => {
    ...
    // We use onSnapshot for real-time updates as requested (ability to update without changing QR)
    const unsubscribe = onSnapshot(q, (snapshot) => {
      if (!snapshot.empty) {
        setProfile({ id: snapshot.docs[0].id, ...snapshot.docs[0].data() } as BusinessProfile);
      } else {
        setProfile(null);
      }
      setLoading(false);
...
END:VCARD`;
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-[#FFFFFF] flex items-center justify-center font-sans tracking-wide">
      <div 
      ...
      {/* Mobile Mockup Frame (centered) on large screens, full screen on mobile */}
      <div className="w-full h-[100dvh] sm:w-[360px] sm:h-[680px] sm:bg-[#000] sm:rounded-[48px] sm:p-[10px] relative sm:shadow-[0_0_0_2px_#333,0_30px_60px_rgba(0,0,0,0.8)] z-10 transition-all">
        ...
        <div className="fixed sm:absolute bottom-[24px] left-[24px] right-[24px] h-[64px] bg-black/40 backdrop-blur-3xl border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.6)] rounded-[32px] flex items-center justify-around px-4 z-50">
           ...
           {profile.contact.address ? (
             ...
             <div className="w-[40px] h-[40px] rounded-full bg-white/5 flex flex-col items-center justify-center text-white/20 transition shadow-sm">
               <Navigation className="w-[18px] h-[18px]" />
             </div>
           )}
        </div>
